...
class App extends React.PureComponent {
     ...
     */
     lisaaJoukkue(tiedot) {
      ...
      tiedotKopio.leimaustapa = apulista;
      apulista = [];
      for (let key in tiedot.jasenet) {
          if (tiedot.jasenet[key].trim()) {
              apulista.push(tiedot.jasenet[key].trim());
          }
      }
      tiedotKopio.jasenet = apulista;
  ...
  */
  etsiId(taulu) {
      ...
      for (let i = 0; i < taulu.length; i++) {
          if (taulu[i].id >= suurin) { suurin = taulu[i].id + 1; }
      }
      return suurin;
  }
...
class LisaaJoukkue extends React.PureComponent {
  constructor(props) {
    ...
    // Objekti , jossa on oletus arvot kentille jäsenet
    this.jasenet = {}
    for (let i = 0; i < 5; i++) {
        this.jasenet["jasen"+i] = "";
    }

    // Objekti, jossa on oletus arvot kentille leimaustavat
    this.leimaustavat = {}
    ...
    // Objekti, jossa ensimmäinen valinta 
     ...
     */
     tarkista(event) {
      ...
      if (kentta === "nimi") {
          if (!arvo.trim().length) {
              obj.setCustomValidity("Syötä nimi");
              obj.reportValidity();
          } else {
      ...
      if (kentta === "jasenet") {
          ...
          obj.setCustomValidity("Jäsenen nimi on syötettävä  vähintään kenttiin 1 ja 2");
...
class Sarja extends React.PureComponent {
  render() {
      ...
      for (let sarja of this.props.sarjat) {
          // Ensimmäisen radiobuttoni defaulttina valittu.
          if (i < 1) {
